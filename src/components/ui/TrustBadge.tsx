...
export function TrustBadge({ className = '' }: TrustBadgeProps) {
  return (
    <div className={`inline-flex items-center gap-2 rounded-full border border-[var(--cream-3)] bg-white px-4 py-2 shadow-sm ${className}`}>
      <StarRating size="md" />
      ...
      <span className="font-condensed text-xs text-[var(--grey)]">+ de 1000 clients satisfaits</span>
    </div>
  )
}
