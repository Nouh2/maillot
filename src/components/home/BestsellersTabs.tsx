          ...
          <div className="mt-3 grid grid-cols-2 gap-2 md:hidden">
            <Link href={collectionHref} className="relative row-span-2 block overflow-hidden bg-[var(--cream-2)]" style={{ borderRadius: 2, minHeight: 200 }}>
              {activeTab === 'Tous' ? (
                ...
                <ExternalProductImage src={featured.photos[0]} alt={featured.name} fill unoptimized fallbackMode="placeholder" className="object-cover" sizes="45vw" />
              ) : null}
              ...
              return (
                <Link key={product.id} href={`/shop/${product.slug}`} className="relative block overflow-hidden bg-[var(--cream-2)]" style={{ borderRadius: 2 }}>
                  <div className="relative" style={{ aspectRatio: '3/4' }}>
                    {product.photos[0] ? <ExternalProductImage src={product.photos[0]} alt={product.name} fill unoptimized fallbackMode="placeholder" className="object-cover" sizes="45vw" /> : null}
          ...
          <div className="mt-3 hidden md:grid md:grid-cols-4 lg:grid-cols-4 gap-4">
            <Link href={collectionHref} className="relative block overflow-hidden bg-[var(--cream-2)]" style={{ borderRadius: 2 }}>
              {activeTab === 'Tous' ? (
                ...
                <ExternalProductImage src={featured.photos[0]} alt={featured.name} fill unoptimized fallbackMode="placeholder" className="object-cover" sizes="25vw" />
              ) : null}
              <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(28,23,18,0.88) 0%, transparent 55%)' }} />
            ...
            {rest.slice(0, 3).map((product) => {
              ...
              return (
                <Link key={product.id} href={`/shop/${product.slug}`} className="relative block overflow-hidden bg-[var(--cream-2)]" style={{ borderRadius: 2 }}>
                  <div className="relative" style={{ aspectRatio: '3/4' }}>
                    {product.photos[0] ? <ExternalProductImage src={product.photos[0]} alt={product.name} fill unoptimized fallbackMode="placeholder" className="object-cover" sizes="25vw" /> : null}
