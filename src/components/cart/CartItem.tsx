      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex items-center border border-[var(--cream-3)]">
            <button aria-label="Diminuer la quantité" onClick={() => updateQty(item.product_id, item.size, item.qty - 1)} className="px-2 py-1 hover:bg-[var(--cream)]">-</button>
            <span className="px-3 py-1 text-sm">{item.qty}</span>
            <button aria-label="Augmenter la quantité" onClick={() => updateQty(item.product_id, item.size, item.qty + 1)} className="px-2 py-1 hover:bg-[var(--cream)]">+</button>
          </div>
          <p className="font-condensed font-semibold">{(item.price * item.qty).toFixed(2)} €</p>
        </div>
      </div>
      <button aria-label={`Supprimer ${item.name} du panier`} onClick={() => removeItem(item.product_id, item.size)} className="text-[var(--grey)] hover:text-[var(--terra)] transition-colors flex-shrink-0">
